...
-> Form.Decoder.Container {

    func parseHelp(_ container: inout Form.Decoder.Container, _ path: [String], _ value: String) {
        switch container {
        case .keyed(let originalParams):
            ...
            if path.count == 1 {
                ...
                var values = Form.Decoder.Container.unkeyed(params[key]?.values ?? [])
                parseHelp(&values, Array(path[1...]), value)
                params[key] = values
            } else {
                var values = Form.Decoder.Container.keyed(params[key]?.params ?? [:])
                parseHelp(&values, Array(path[1...]), value)
                params[key] = values
            }
        ...
        case .unkeyed(let originalValues):
            ...
            if path.count == 1 {
                ...
                var nestedValues = Form.Decoder.Container.unkeyed([])
                parseHelp(&nestedValues, Array(path[1...]), value)
                values.append(nestedValues)
            } else {
                var params = Form.Decoder.Container.keyed([:])
