...
struct URLRoutingFormTests {
  ...
  struct RoundTripTests {
    ...
    func testFormCodingRoundTripsWithMatchingDateStrategies() throws {
      ...
      #expect(
        abs(decoded.createdAt.timeIntervalSince1970 - original.createdAt.timeIntervalSince1970)
          < 1.0
      )
      #expect(
        abs(decoded.lastLogin!.timeIntervalSince1970 - original.lastLogin!.timeIntervalSince1970)
          < 1.0
      )
    }
  }
