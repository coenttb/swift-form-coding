...
struct FileUploadBasicTests {
    ...
    func uniqueBoundaryGeneration() {
        let upload1 = Multipart.FileUpload(
            fieldName: "test1",
            filename: "test1.txt",
            fileType: .text
        )
        let upload2 = Multipart.FileUpload(
            fieldName: "test2",
            filename: "test2.txt",
            fileType: .text
        )
...
struct ImageTypeTests {
    ...
    func webpValidation() async throws {
        ...
        let validWebP =
            "RIFF".data(using: .ascii)! + Data([0x20, 0x00, 0x00, 0x00])  // File size (little endian)
            + "WEBP".data(using: .ascii)!
        let invalidWebP =
            "RIFF".data(using: .ascii)! + Data([0x20, 0x00, 0x00, 0x00]) + "WAVE".data(
                using: .ascii
            )!  // WAV file
    ...
    func heicValidation() async throws {
        ...
        let validHEIC =
            Data([0x00, 0x00, 0x00, 0x20])  // Size
            + "ftyp".data(using: .ascii)! + "heic".data(using: .ascii)!
            + Data(repeating: 0x00, count: 16)
        let invalidHEIC =
            Data([0x00, 0x00, 0x00, 0x20]) + "ftyp".data(using: .ascii)! + "jpeg".data(
                using: .ascii
            )!  // Wrong brand
    ...
    func avifValidation() async throws {
        ...
        let validAVIF =
            Data([0x00, 0x00, 0x00, 0x20])  // Size
            + "ftyp".data(using: .ascii)! + "avif".data(using: .ascii)!
            + Data(repeating: 0x00, count: 16)
        let invalidAVIF =
            Data([0x00, 0x00, 0x00, 0x20]) + "ftyp".data(using: .ascii)! + "heic".data(
                using: .ascii
            )!  // Wrong brand
...
struct ErrorHandlingTests {
    ...
    func errorDescriptions() {
        ...
        let invalidContentError = Multipart.FileUpload.MultipartError.invalidContentType(
            "invalid/type"
        )
